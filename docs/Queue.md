...
- **Record touches are idempotent and never shorten a visit.** `updated` is the time the API answered, not the time the worker writes, and MongoDB keeps the larger of the stored and the delivered value (`$max`), so a redelivered touch sets the same value and a touch replayed behind a later one changes nothing. **Action touches do not have that guarantee:** `updateAction` replaces `key`, `value`, `details` and `updated` together (`$set`), because the three belong to one update and cannot be merged field by field. A redelivered touch is harmless, but one replayed behind a later touch of the same action restores the earlier values until the next update arrives.
